...
;;
;; To use these tools:
;; 1. Load claude-code.el first to get the emacsmc-defmcp macro
...
;;
;; Functions defined with emacsmc-defmcp are automatically discovered
...
(emacsmc-defmcp mcp-hello-world (name)
...
(emacsmc-defmcp mcp-set-allowed-variable (variable-name value)
...
(emacsmc-defmcp mcp-call-allowed-function (function-name &rest args)
...
(emacsmc-defmcp mcp-get-variable-value (variable-names)
...
(emacsmc-defmcp mcp-get-agenda (&optional agenda-type)
...
(emacsmc-defmcp mcp-org-agenda-todo-batch (batch-updates &optional agenda-type)
...
(emacsmc-defmcp mcp-org-schedule-todo (org-file heading-text schedule-date &optional remove-schedule)
...
(emacsmc-defmcp mcp-org-archive-todo (org-file heading-text &optional archive-location)
...
(emacsmc-defmcp mcp-org-capture (&optional template-key title content immediate-finish)
...
(emacsmc-defmcp mcp-org-get-all-todos (&optional include-done org-files)
...
(emacsmc-defmcp mcp-org-agenda-goto (target-type target &optional agenda-type context-lines)
...
(emacsmc-defmcp mcp-open-file (file-paths)
...
(emacsmc-defmcp mcp-emacs-search (pattern &optional type predicate)
...
(emacsmc-defmcp mcp-emacs-describe (symbol-names &optional type)
...
(emacsmc-defmcp mcp-emacs-keymap-analysis (buffer-names &optional include-global)
...
(emacsmc-defmcp mcp-emacs-buffer-info (buffer-names &optional include-content include-variables)
...
(emacsmc-defmcp mcp-check-parens (file-paths)
...
(emacsmc-defmcp mcp-get-workspace-buffers (&optional workspace-name)
...
(emacsmc-defmcp mcp-rename-workspace (workspace-identifier new-name)
...
(emacsmc-defmcp mcp-create-workspace (workspace-name)
...
(emacsmc-defmcp mcp-delete-workspace (workspace-identifier)
...
(emacsmc-defmcp mcp-move-protected-buffers-to-workspace (source-workspace target-workspace)
...
(emacsmc-defmcp mcp-setup-workspace-layout (workspace-name layout)
...
(emacsmc-defmcp mcp-view-buffer (buffer-names)
...
(emacsmc-defmcp mcp-count-parens (file-path start-line end-line)
...
(emacsmc-defmcp mcp-check-parens-range (file-path start-line end-line)
...
(emacsmc-defmcp mcp-show-paren-balance (file-path start-line end-line)
